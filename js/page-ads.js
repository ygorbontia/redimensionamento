 ...
 * @param {'f'|'s'} params.suffix           sufixo: 'f' para feed, 's' para story
...
uploadInput.addEventListener('change', async () => {
  ...
  // Valida e ordena apenas JPEG/PNG
  ...
  imagesData.forEach(({ fileName, image }) => {
    outputContainer.appendChild(
      renderCanvas({ width: 1350, height: 1350, image, fileName, suffix: 'f', imageOnTopHeight: 1350 })
    );
    outputContainer.appendChild(
...
downloadAllBtn.addEventListener('click', async () => {
  ...
  // Restaura botão original após breve intervalo
  setTimeout(() => {
